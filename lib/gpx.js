...
function getProperties(node) {
  ...
  // Parse additional data from gsak extension(s)
  ...
  for (let i = 0; i < extensions.length; i++) {
    ...
    // Parse additional data from c:geo extension(s)
    const extensions2 = node.getElementsByTagNameNS(
      "http://www.cgeo.org/wptext/1/0",
      "*"
    );
    for (let j = 0; j < extensions2.length; j++) {
      const extension2 = extensions2[j];
      // Ignore nested extensions, like those on routepoints or trackpoints
      if (extension2.parentNode.parentNode === node) {
        prop[extension2.tagName.replace(":", "_")] = nodeVal(extension2);
      }
    }
